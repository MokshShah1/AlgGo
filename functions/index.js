/**
 * AlgGo AI proxy as a Firebase Cloud Function (2nd gen).
 *
 * This is the production counterpart to server/ai-server.mjs. The deployed web
 * app calls it same-origin via Hosting rewrites (/ai/** and /health), so the
 * OpenAI key never ships to the browser. The key is stored as a Firebase secret
 * (OPENAI_API_KEY), not in the bundle or the repo.
 */
...
/** Strip em/en dashes from AI copy (they read as "AI slop"). */
...
async function route(apiKey, path, body) {
  if (path === "/ai/hint") {
...
export const api = onRequest(
  ...
  async (req, res) => {
    ...
    if (!apiKey) {
      res.status(503).json({ error: "AI not configured: set the OPENAI_API_KEY secret." });
      return;
    }
    ...
    try {
      ...
      const { code, payload } = await route(apiKey, path, body);
